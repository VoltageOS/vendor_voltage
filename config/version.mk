...
ifeq ($(TARGET_BUILD_VARIANT),OFFICIAL)
    ...
    PROD_VERSION += VOLTAGE-$(PLATFORM_VOLTAGE_RELEASE)-$(TARGET_DEVICE)-$(shell date +%m%d%H%M)-OFFICIAL
else
    PLATFORM_VOLTAGE_VERSION := $(PLATFORM_VOLTAGE_RELEASE)
    PROD_VERSION += VOLTAGE-$(PLATFORM_VOLTAGE_RELEASE)-$(TARGET_DEVICE)-$(shell date +%m%d%H%M)-UNOFFICIAL
endif
